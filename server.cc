...
//const float temper = 10.22f;
//const float temper = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/100));;
...
main(void) {
  ...
  coap_str_const_t *ruri = coap_make_str_const("temp");
  coap_startup();
  ...
  coap_register_handler(resource, COAP_REQUEST_GET,
                        [](auto, auto,
                           ...
                           auto,
                           coap_pdu_t *response) {
                          float temper = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/100));
                          coap_show_pdu(LOG_WARNING, request);
                          coap_pdu_set_code(response, COAP_RESPONSE_CODE_CONTENT);
                          coap_add_data(response, 4, (uint8_t *)(&temper));
